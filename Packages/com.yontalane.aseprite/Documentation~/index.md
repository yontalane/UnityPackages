...
| **FrameBounds**              | The bounds, in local space (the same space used by Colliders'/Triggers' offset and size), of each frame's visible (non-transparent) artwork, indexed by the frame's position in the Aseprite file. |
